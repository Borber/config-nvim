...
- 配置偏向项目工作流：starter 负责入口，`mini.files` 负责文件树，`config.recent_projects` 负责最近路径和项目打开，`mini.sessions` 负责会话，fzf-lua / Trouble / Overseer 分别承担查找、诊断列表和任务运行。
...
- 公共路径、buffer、浮窗、fzf-lua picker 等能力优先复用 `lua/util/` 下已有 helper。
...
- 先判断行为属于哪一层，再修改：
  ...
  - `config.recent_projects`：最近路径记录、打开文件/目录、切换 cwd。
...
- `Config` 这类表示目录/项目的入口要继续走 `recent_projects.open_path(..., { record = false })` 和 session 恢复流程，不要改成单文件打开。
...
- 浮窗边框、标题和高亮优先复用 `lua/util/float.lua`，让 fzf-lua、Noice、LSP、mini.clue、Overseer 等入口保持一致。
- `signcolumn` 默认保留，避免诊断、git sign 或书签 sign 出现时正文跳动。
- picker 体验优先使用 fzf-lua 浮窗风格；不要新增数字列表式 fallback，除非用户明确要求。
...
- Overseer 模板和 action 选择使用 fzf-lua picker；任务启动时不默认展开输出，失败后自动打开错误输出。
...
- 修改 starter、mini.files、config.recent_projects、mini.sessions 等交互边界后，优先运行：
